---
title: "Fonction utilisant les données ecologiques des relevés de l'IFN"
...
Lors de l'inventaire réalisé par l'IFN de nombreuses données sont mesurées concernant l'écologie, la topologie et l'hydrologie. De plus, un inventaire floristique est réalisé. Ce code a pour objectif d'explorer les données apportées par l'IFN et voir ce qu'il est possible de faire avec pour aider le gestionnaire forestier.

Les trois premières fonctions permettent à l'utilisateur de choisir sa zone d'étude.

# Get_import_zone

La fonction get_import_zone() permet d'importer une zone d'étude qui s'appelle "shp_etude". Ce fichier est importé ou dessiné par l'utilisateur sur R via le fonction drawfeatures(). Le dessin est transformé en shape file puis les coordonnées sont transformées en Lambert 93. A chaque étape une vérification est effectué pour s'assurer que la transformation se déroule correctement.
...
La fonction get_buffer_zone() est utilisée pour créer une zone tampon (buffer) autour d'une zone d'étude géographique (contenue dans l'objet shp_etude). Cette zone tampon est utilisée pour sélectionner les placettes qui se trouvent à l'intérieur de la zone tampon. Ainsi on récupère les IDP de chaque placette.
...
La fonction get_read_map permet de visualiser via une carte interactive la zone d'étude qui a été selectionné avec toutes les placettes présentes dans cette zone.
...
La fonction get_pla_eco() est utilisée pour extraire et filtrer les données écologiques associées aux placettes qui se trouvent à l'intérieur de la zone tampon définie précédemment. Dans ce cas on prend l'IDP, la topographie, l'humus, le type de sol et sa texture, le type de roche mère ainsi que des informations sur la présence de point d'eau ou de rivière sur la placette. Certains paramètres peuvent être enlevés ou rajoutés en fonction des besoin de l'utilisateur.
...
La fonction filtre_peuplement() permet de filtrer le peuplement de diverses manières. Dans cette fonction trois type de filtres sont proposés. Un premier filtre permet de choisir la sylvoécorégion que l'on veut étudier en entrant dans la console son nom. Le deuxième permet en tapant le code de typology donné de sélectionner que les placettes correspondant à la topologie demandée. De la même manière le troisième filtre permet de choisir le type de peuplement via le code fourni par l'inventaire de l'IFN. Bien évidemment d'autre filtres peuvent être ajouté. Cependant plus on ajoute de filtres, moins on a de donné. Ce qui peut ce qui peut poser problème pour l'interprétation des résultats.

Ces filtres pourraient être intégré aux fonctions d'accroissement pour ainsi avoir des accroissements qui correspondrait au miens aux attentes de l'utilisateur.
...
filtre_peuplement <- function(){
  ...
  placette_etude_eco <<- placette_ecoregion_F
...
# get_proportion_eco et get_data_value

La fonction get_proportion_eco() permet sur les placettes étudiés de calculer la proportion de chaque type d'humus ou de type de sol. Ensuite, il est préférable d'utiliser la fonction get_data_value() qui permet d'associer aux codes de l'IFN le libellé et la définition associés. Cela permet de faciliter la compréhension de l'utilisateur.
...
La fonction get_groupe_flore() permet de créer des groupes de plantes caractéristiques d'un milieu, dans ce cas-là les plantes qui ont été choisies sont celles considérées comme caractérique des plaines lorraines. Cela peut permettre de déterminer des stations. De plus, de la même manière il pourrait être possible de mettre en évidence la présence de plantes rares ou protégées.
...
Cette fonction permet de calculer par placettes le nombre de plantes de chaque groupe présentes.
...
get_data_idp <- function(){
  ...
  for (groupe in names(liste_groupe_eco)) {
    ...
    groupe_plantes <<- placette_sol_flore %>%
      inner_join(df_indicateur, by = c("IDP", "CD_REF")) %>%
      group_by(IDP) %>%
...
Cette fonction permet d'obtenir l'ensemble des données issues des fonctions présentées précédemment. Les données sont ainsi enregistrées dans l'environnement global et peuvent ainsi être consultées librement.
...
# Conclusion et possibilité d'amélioration

Les données de l'IFN permettent d'apporter de nombreuses informations pour chaque placette. Cependant ce peut aussi être leur défaut. En effet certaines données sont anciennes et les placettes doivent être remesurées mais il faut attendre plusieurs années avant que de nouvelles mesures soient effectuées. Les possibilités d'amélioration sont multiples. Notamment l'ajout de nouveaux filtres pour améliorer la véracité des calculs d'accroissement pour un type de forêt choisie. Il semble aussi possible mais laborieux de coder les clés de détermination pour chaque sylvoécorégions. Cependant il faut prendre la distance de 1 à 2km minimum entre chaque points, les contextes forestier peuvent ainsi être différents même pour des distance de cet ordre.
